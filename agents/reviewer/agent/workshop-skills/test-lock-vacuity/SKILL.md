...
4. **Tautology check on any "discrimination" sub-check.** Find the block that claims to prove the anchors discriminate. If it writes a fixed dummy text and greps that, its `fail` branch can never fire and its `ok` line disproves nothing. The probe must derive from the real artifact (mutate a copy of it), not from a constant. Done when you can state whether the sub-check can ever fail.
