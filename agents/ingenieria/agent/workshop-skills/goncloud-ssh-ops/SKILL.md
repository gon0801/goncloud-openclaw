...
5. Deploying an app change — inspect the live layout first, then take one branch:
   - From the Mac repo: `git fetch origin` and confirm the approved SHA is in `origin/master` (`git log -1 --oneline origin/master` is the merge, or `git merge-base --is-ancestor <sha> origin/master`). Agents cannot merge; never archive a feature branch.
   - Layout check: `git -C /mnt/data/appdata/<app> status --short` + `git -C /mnt/data/appdata/<app> rev-list --left-right --count HEAD...origin/master`. Clean and on the deploy branch → `git pull origin <branch>` (accounting-style). Dirty or far behind (orbit: 187 commits + local edits; its `docs/DEPLOY.md` says the server is not a checkout) → do NOT pull; ship from the repo: `git archive --format=tar origin/master app Dockerfile .dockerignore pyproject.toml uv.lock tools/fabrica_campanas.py | ssh gonserver 'cd /mnt/data/appdata/orbit && cp -a app app.bak-predeploy-$(date +%Y%m%d-%H%M) && tar -xf -'` (archive keeps LF; back up `app/` first), then confirm shipped files match `git show origin/master:<path> | md5`.
