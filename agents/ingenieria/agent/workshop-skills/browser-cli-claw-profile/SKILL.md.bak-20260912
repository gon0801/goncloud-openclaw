...
4. NEVER run `reset-profile` on `claw`: it moves the profile's browser data to Trash and wipes every logged-in session (Seller Central, Seller Flex, Gmail, Mercado Libre).
   One session drives the claw browser at a time: the session that owns the task. Never split browser work across sessions or fan it out to helpers, and never touch the browser while a session you dispatched or spawned could still be on it — a sub-agent that times out leaves its own children running.
...
5. On the Windows gateway host every exec of the browser CLI needs `timeoutSeconds >= 60`: the CLI alone takes 10-23 s to start, so a 15-20 s timeout kills it before it connects.
