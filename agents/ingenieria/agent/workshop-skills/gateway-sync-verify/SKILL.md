...
You must confirm a merged SHA is live on the gateway: autopilot gates, and summa-gate/tablero-runbook/workshop-skill/watchdog changes. Since 2026-09-22 `C:\Users\ehven\.openclaw` is live state, **not** a git clone, and the old 2-hourly sync is disabled: a merge reaches the runtime only when the operator runs `docs/runbooks/publicar-runtime-windows.md`.
...
4. No publication containing the merge yet → report "merged, not published" and name the operator step. Never call it deployed because the merge exists.
...
6. Fire test after a plugin/guard change: run ONE harmless read-only exec on the node (`gh pr view <n> -R <repo> --json state`). Success proves the deployed plugin loaded and exec is not broken. A denial or failure right after the publication means the change broke the hook path: the rollback is `publish-selected.mjs … --rollback` with that publication's folder (runbook «Revertir»); declare it with the verbatim error. The guard blocking a forbidden pattern is the healthy signal; a legit read-only command failing is the alarm.
