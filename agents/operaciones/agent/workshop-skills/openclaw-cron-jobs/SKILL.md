...
9. Every business job carries `--failure-alert` to David (`--failure-alert-channel telegram --failure-alert-account-id default --failure-alert-to 6470689715 --failure-alert-after 1 --failure-alert-cooldown 1h --failure-alert-mode announce`); delivery `announce -> last` on an isolated job fails closed ("Refusing implicit isolated cron delivery") — use `--no-deliver` or an explicit `--to` instead.
