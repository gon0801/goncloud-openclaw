...
description: Create, patch, or force-test OpenClaw cron automations (single-line prompt files, scratch, run monitoring, CLI-locked fallbacks, failure alerts) on the Windows gateway. Use when adding, editing, or force-testing scheduled jobs, or when a job appears to have run without doing its work.
---
...
1. Check for duplicates with `openclaw cron list --all` before creating a job.
...
5. If exec denies the CLI — signatures: `approval cannot safely bind this command` / `approval script operand changed`, observed after gateway restarts, with `Approved executables: none` in context (that line alone does NOT mean exec is blocked: try the real command first) — report the exact command and error to the owner and do not route around the denial (no other host, no other profile). The first-class `automations` tool is caller-scoped and will not list these jobs, and the `openclaw` system-expert tool cannot edit cron payloads. Read `exec-locked-fallback.md` for the verified fallback paths before improvising.
...
9. Every business job carries a failure alert to David — but know what it does not cover: it fires after N consecutive job errors, so a run that ends `ok` while having done nothing sends no alert (crashes, not missing work). Alert setup: `--failure-alert*` exists only on `cron edit`, NOT on `cron create` — create the job, then apply `openclaw cron edit <job-uuid> --failure-alert --failure-alert-channel telegram --failure-alert-account-id default --failure-alert-to 6470689715 --failure-alert-after 1 --failure-alert-cooldown 1h --failure-alert-mode announce`, and confirm the `failureAlert` block in the returned JSON. Delivery `announce -> last` on an isolated job fails closed ("Refusing implicit isolated cron delivery") — use `--no-deliver` or an explicit `--to` instead.

## Completion check

- `openclaw cron list --all` shows the job with the expected schedule and next-run time.
