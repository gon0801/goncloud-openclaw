...
- This skill is about the **Mac's** tunnel. If the app that must answer is the OpenClaw app on the phone rather than Screens to the Mac, the diagnosis is different: reaching the VPN server proves nothing about reaching the gateway, because what the gateway answers on is set by `gateway.bind` (check it with `openclaw config get gateway.bind`) and not by the VPN. Use `gateway-mobile-pairing`.
