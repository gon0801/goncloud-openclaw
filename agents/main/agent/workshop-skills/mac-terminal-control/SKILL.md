...
description: Operate David's Mac (macOS node "David's MacBook Pro") to read or type into Terminal tabs. Use when asked to see or control what's running in a Terminal window, when a CLI agent's tab stopped producing output, when an agent hits its usage limit and the task must move to another open one, when node permissions/osascript behave unexpectedly, or when a headless CLI-agent invocation fails with `COMPANION_APP_UNAVAILABLE`. Produces the tab→tty map, the correct osascript calls, how to unstick a stalled TUI, the substitute-agent rule, and the permission-state decision rules.
---
