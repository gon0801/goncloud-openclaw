...
2. From the lane's worktree:
   `PATH=/opt/homebrew/bin:$PATH bash /Users/dn/dev/summonaikit-claude/tools/saikit-merge.sh --dry-run` → prints `DRY-RUN: gate en verde`; the same command with `--auto` performs the squash merge after all gates pass.
...
2. Gateway SHAs: `git -C <clone> log -1 --format=%H` per repo must equal the merged SHA. Treat gateway/workspace deployment as a separate operation; verify the published SHA directly before reporting it live.
