...
- DNS: A record `<name> → <tunnel-ip>`, DNS-only. Cloudflare shows "DNS only - reserved IP" for RFC1918 targets — that means already correct, nothing to switch. TXT pitfalls (each cost a failed validation): the Name must keep the leading underscore (`_acme-challenge.<name>` — without it the name NXDOMAINs in all variants); paste the value with NO quotes (a quoted value mismatches).
