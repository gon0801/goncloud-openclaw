...
NEW1=""; NEW2=""; NEW3=""
send_one() { # $1=idx $2=chat -> imprime MID o nada
  local idx="$1" chat="$2" R MID
  R="$(curl -sS -X POST "$TG_SCHEME://$TG_API_HOST/bot${TELEGRAM_SALES_BOT_TOKEN}/sendPhoto" \
  ...
  if echo "$R" | grep -Eq '"ok":[[:space:]]*true'; then
    echo "$R" | grep -Eo '"message_id"[[:space:]]*:[[:space:]]*[0-9]+' | head -1 | grep -Eo '[0-9]+'
  else
    echo "ERR_CHAT chat#$idx: $(echo "$R" | head -c 120)" >&2
  fi
}
