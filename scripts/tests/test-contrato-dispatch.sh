...
for ws in workspace-implementer workspace-verifier workspace-reviewer workspace-adversary; do
  ...
  printf '%s\n' "$nunca" | grep -q 'no usa rutas directas de merge' || { echo "FALLO: $ws permite una ruta directa"; fails=$((fails+1)); }
  printf '%s\n' "$nunca" | grep -q 'saikit-merge.sh --auto' || { echo "FALLO: $ws no permite el gate autónomo"; fails=$((fails+1)); }
done
