...
| OpenAI cuota / codex | M1 patches preparados | `docs/patches/modelos-*.json5` + dry-run log |
| pathPrepend warning / noise | N1 docs + `scripts/mac/shot.sh` | `docs/patches/n1-mac-node.md`, `shot-evidence.txt` |

## Para el lead

1. Aplicar M1 fase1 (en el **gateway**, no desde este remote CLI): ver `docs/patches/README.md`. Luego V1 pings.
2. Aplicar fase2-main solo despues del estreno.
...
- Toggle dedicado allowPrivateNetwork para `view_image` (distinto de model-provider).
