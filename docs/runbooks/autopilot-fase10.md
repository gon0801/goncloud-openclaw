...
> **Autoridad vigente (2026-09-21, entrega sin sello):** aprobar y mergear se rige por el recibo del PR (`saikit-entrega.v1`, `loop-autopilot.md` §6). Donde este runbook describe el sello del kit, el `harness-state.env`, el sondeo de sesión o la ceremonia del veredicto sellado, eso es historia de la corrida que ya corrió —se conserva tal como se ejecutó— y NO es una instrucción: el merge no depende de sello, sesión, host ni cwd. Tampoco manda la «revisión completa de la sección 10 contra la DoD» que aquí se presupuesta: el §10 vigente cierra con aceptación, instalación, simulacro y `cierre-de-fase.sh`, sin repetir la revisión de código.
