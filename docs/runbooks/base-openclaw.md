...
**No hay sync automático.** Desde la separación del runtime (22 de septiembre de 2026, `docs/spec/00-project-spec.md` «Propiedad del runtime Windows») la tarea `GoncloudRepoSync` está deshabilitada y `scripts/sync-repos.ps1` es obsoleto: nada baja solo al gateway. `goncloud-openclaw` se publica a mano, con autorización del dueño, por [publicar-runtime-windows.md](publicar-runtime-windows.md). Los tres `workspace-*` de la tabla **ya no son clones git en el runtime** (leído 2026-09-24: `workspace` es un git vacío sin remote, los otros dos no son repos) y no tienen camino de publicación: sus filas dicen dónde vivían, y cualquier compuerta que compare SHAs con ellos queda `unknown` hasta que se decida uno.
