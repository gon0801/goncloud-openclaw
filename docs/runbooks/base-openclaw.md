...
| `LISTO: todas las condiciones del gate estan en verde para <sha> (PR <pr>)`, código 0 | Es la corrida **sin flags**, la que sí imprime `LISTO` | Cualquier agente corre `--auto`, que repite CI, CodeRabbit, recibo y SHA y mergea en squash con `--match-head-commit <sha>` |
