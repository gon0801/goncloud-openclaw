...
| Rol | Quién | Qué hace |
|---|---|---|
| **claw** | el agente `main` del gateway | Elige al lead de su lista de preferencia y lo lanza con `scripts/lanzar-lead.sh`. Lo vigila por tmux y lo relanza si se cae. Al aviso `closed` de una sesión que figura en una **corrida abierta** (`~/.local/state/corridas/*/registro.json` con `"estado": "abierta"`, en la Mac), la relanza una vez con `/Users/dn/bin/corrida.sh lanzar-sesion <id> <rol> <cli> <dir> --nombre <sesión>`, aunque no haya fase activa: el registro de la corrida, no la fase, es lo que vuelve relanzable una sesión (medido 2026-09-24, simulacro 9.9: sin esta regla, las sesiones caídas de una corrida sin fase se quedaban muertas). Le presta su exec para los comandos del host Windows. Puede mergear y desplegar sin permiso adicional. |
| **lead** | **un rol, nunca un nombre de modelo**: cualquier CLI que Claw pueda lanzar de forma verificada en tmux; Claw elige por preferencia y disponibilidad y puede relevarlo a mitad de fase | Spike, encargos (`BRIEF.md`), lanzar implementadores, auditar, revisión cruzada, registrar evidencia, mergear por GitHub, desplegar, escribir progreso, cerrar y mandar los mensajes a David. **No escribe código de producto**; sí implementa los carriles de solo lectura que el plan le asigne. |
...
| **CodeRabbit** | bot en GitHub | Revisa cada PR. Su aprobación es obligatoria antes del merge; si no responde, el PR espera y los otros carriles continúan. |
...
**El sentinel `-saikit:autopilot`** arma las instrucciones de ejecución del turno. Los merges y despliegues siguen el permiso permanente para todos los agentes y los checks del PR; no requieren un recibo adicional.
...
5. **Base al día con merge, nunca con rebase** (loop §3 paso 9). Si `main` avanzó mientras el PR esperaba: `git fetch origin && git merge origin/main`, push normal, y verifica CI y CodeRabbit sobre el SHA actual del PR. El commit de merge no cuenta como commit ajeno en la compuerta.
...
7. **Merge y deploy sin permiso adicional.** Cualquier agente usa el flujo normal de GitHub con CI y CodeRabbit aprobados. No se exige el kit ni recibos del lead.
8. La evidencia de calidad queda en el PR y en CI.
...
**El loop de entrega no se reescribe aquí: se cita por número.** Secciones de `docs/runbooks/loop-autopilot.md`, en orden: 1 Roles · 2 El contrato de reporte · 3 El loop por tarea · 4 Política de rondas de revisión cruzada · 5 PRs y CodeRabbit · 6 Merge del PR · 7 Despliegue y configuración del gateway · 8 Progreso escrito, no contado · 9 El lead es reemplazable · 10 Revisión de cierre de fase · 11 Lo que lleva un runbook de fase, y lo que no · 12 Atores universales · 13 Cómo se prueba este documento. Manda **el loop que esté en `origin/main` cuando lo consultas**; se prueba con `bash scripts/tests/test-loop-autopilot.sh`.
...
**Merge por cualquier agente.** Ejecuta `gh pr merge <pr> --squash --match-head-commit <sha>` con CI y CodeRabbit aprobados. Comprueba `MERGED`. No se requiere el kit, recibo ni autorización adicional.

**Mergear no despliega.** Cualquier agente puede publicar por [publicar-runtime-windows.md](publicar-runtime-windows.md), sin esperar una ventana de cron ni permiso adicional.

**Compuerta de publicación**, cuando la fase deja algo en el runtime: la salida del paso 5 del runbook de publicación (`N selected files published; …` o `N selected files already published`) y la fuente del host conteniendo el SHA: `git -C C:\Users\ehven\src\goncloud-openclaw merge-base --is-ancestor <SHA> HEAD; if ($LASTEXITCODE -eq 0) { 'PRESENTE' } else { 'AUSENTE' }` (sintaxis de Windows PowerShell 5.1: sin `&&` ni `||`). Un cambio que solo toca `scripts/` o `docs/` no pasa por la allowlist: su compuerta es solo la fuente `PRESENTE`. Cualquier agente completa la publicación sin permiso adicional y verifica el resultado.

**El camino de emergencia: la reversa.** Crea una rama desde `origin/main`, ejecuta `git revert --no-edit <merge_commit>`, abre un PR con el error reproducido y mergea por el flujo normal tras CI y CodeRabbit. Si hay conflictos, resuélvelos y verifica el resultado; no se exige trailer del kit ni recibo del lead.

**El ítem de cierre** cierra las celdas `Status` de `Plans.md` con el token literal **`cc:完了`** y el SHA de squash, **solo con evidencia**: una tarea con PR lleva `cc:完了 [PR #n, merge <sha>]`; una sin PR propio, `cc:完了 [evidencia: <ruta>, PR de cierre #n]` (el PR de cierre no puede citar su propio SHA); lo que no se hizo lleva qué quedó y por qué. Antes va la **aceptación y cierre de fase** de loop §10: la promesa de cada fila recorrida como la recorrería David, la comprobación de instalación y el simulacro; el cierre no repite la revisión de código, que cada bloque ya tuvo en sus rondas. El cierre requiere CI y CodeRabbit aprobados; cualquier agente puede mergear y publicar sin otra autorización.
...
| **CodeRabbit sin cuota, rate limit o sin respuesta en 20 minutos** | Ese PR espera la aprobación de CodeRabbit; se anota el motivo en el cuerpo del PR y en el progreso, y se vuelve a consultar. Los otros carriles y PRs que no dependan de ese merge continúan. **Los comentarios se leen, no solo el check**: medido 2026-09-16, el PR #48 se mergeó con el check en verde y trece comentarios accionables sin leer, seis altos, y cuatro eran candados que daban verde con el defecto puesto. |
