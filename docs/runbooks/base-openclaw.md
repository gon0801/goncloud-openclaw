...
**Propiedad fuente/runtime (desde 16.8, corte operado).** Pre-16.8 el gateway era un clon git: el sync hacía `add -A`, commiteaba su snapshot y hacía pull dentro de `C:\Users\ehven\.openclaw`. Desde 16.8 la fuente vive en el checkout dedicado `C:\Users\ehven\src\goncloud-openclaw` (pinneado al SHA autorizado) y el runtime `C:\Users\ehven\.openclaw` es estado vivo SIN `.git`: el sync actualiza la fuente y despliega por manifiesto (`config/runtime-deploy.v1.json`). La compuerta de despliegue lee `git -C C:\Users\ehven\src\goncloud-openclaw merge-base --is-ancestor <SHA> HEAD` más recibo de deploy, no git en runtime. Estado temporal durante el corte: `GoncloudRepoSync` deshabilitado (sin deploys) mientras el lease de cutover está vigente; el watchdog y el reinicio manual se apartan ante ese lease. Fase 16 reemplaza hacia adelante el contrato de Fase 13 (v2 ` SKILLS ` con commit directo en runtime): desde 16.8 los skills viajan por PR (`SKILLS_PR` pendiente, `SKILLS_DEPLOYED` al mergear); las filas cerradas de Fase 13 y el texto histórico del ledger se preservan como historia.
