...
Versión 1, 2026-09-18 UTC, destilada del runbook de la Fase 10 (`autopilot-fase10.md` v5.1, el último completo, que pasó por tres lectores frescos, un verificador y un revisor). Todo lo que dice «medido» se midió en la Fase 8 o en la 10; la fecha y el lugar están en aquel documento.
...
5. **Un candado que no puede salir rojo no cuenta**: cada candado nuevo llega con su fuga sembrada en `tmp_path` demostrando el rojo. El candado de pureza de `app/precio/` mide con `rglob` y **solo admite excepciones por nombre de archivo, una por carril, cada una con candado propio**; exceptuar la carpeta, quitar el `rglob` o mover los módulos puros devuelve el carril al paso 1 del loop.
...
14. **Sin tope de rondas cruzadas** (regla del dueño del 2026-09-15; desviación de la sección 4 del loop, declarada aquí para todas las fases): se repite mientras la ronda anterior haya encontrado algo, y solo el `APPROVE lead <sha>` mete el PR a la cola. Los PRs de docs (plan, cierre) llevan CI verde + una ronda + `APPROVE`, sin CodeRabbit obligatorio.
