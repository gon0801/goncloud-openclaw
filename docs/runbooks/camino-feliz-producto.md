...
| 3 | Brief al carril | `agents/main/agent/workshop-skills/agent-dispatch/SKILL.md` | El brief trae repo, rama, base, alcance y límites de la meta autorizada. |
...
| 7 | Compuerta de publicación | `docs/runbooks/publicar-runtime-windows.md` y el runbook de despliegue del proyecto | Se autoriza el host, la ventana y el SHA integrado antes de cambiar el runtime vivo. El merge del paso 8 usa la política permanente de la rama base y no pide un go/no-go por PR. |
| 8 | Merge autónomo por cualquier agente tras CodeRabbit | `agents/implementer/agent/workshop-skills/saikit-cierre-pr/SKILL.md` | `saikit-merge.sh --auto` verifica CI, CodeRabbit, recibo y SHA; PR `MERGED` confirmado desde el remoto con state + mergedAt. El cierre posterior del PR (ledger/log, cuando el repo lo lleva) sigue `agents/main/agent/workshop-skills/post-merge-closure/SKILL.md`. |
