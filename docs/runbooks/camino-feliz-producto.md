...
| 3 | Brief al carril | `agents/main/agent/workshop-skills/agent-dispatch/SKILL.md` | El brief trae repo, rama, base, alcance y comprobaciones de calidad. |
| 4 | PR por cadena (implementer → verifier → reviewer) | `agents/main/agent/workshop-skills/agent-dispatch/SKILL.md` | PR abierto, CI verde, cross-review y CodeRabbit limpios. |
...
| 7 | Merge y deploy por cualquier agente | `agents/implementer/agent/workshop-skills/saikit-cierre-pr/SKILL.md` | CI y CodeRabbit aprobados, PR integrado sin permiso adicional. En Windows: merge y luego deploy por `publicar-runtime-windows.md`; comprueba la publicación. |
| 8 | Merge por cualquier agente | `agents/implementer/agent/workshop-skills/saikit-cierre-pr/SKILL.md` | PR `MERGED` confirmado desde el remoto con state + mergedAt. El cierre posterior del PR (ledger/log, cuando el repo lo lleva) sigue `agents/main/agent/workshop-skills/post-merge-closure/SKILL.md`. |
| 9 | Deploy por cualquier agente | `agents/ingenieria/agent/workshop-skills/goncloud-ssh-ops/SKILL.md` | Smoke verde (código HTTP) y service/container arriba. |
...
| Bloqueo | Acción |
|---|---|
| Cadena en cooldown / sin modelos | Re-dispatch como subagente con modelo explícito (agent-dispatch, paso 6). |
| Reviewer rechaza | Reabre el rol anterior (agent-dispatch regla 3); el mapa no decide reintentos. |
| CI rojo por hallazgo mismo 3 rondas | Carril se detiene con diagnostico y PR `autopilot:atorado`. |
| Verify no puede correr (sin secrets / framework) | `n/a` declarado en el PR con la razón; el flujo no se finge verde. |
| Resumen no entrega (turno sin inbound) | Reintentar por owner-report-delivery paso 2 con `ok:true` como cierre. |
