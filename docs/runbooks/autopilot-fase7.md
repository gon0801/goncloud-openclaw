...
**Los comandos del spike (0.4) amplían la lista cerrada del "turno a main"** solo para esta fase y solo estos: `openclaw --version`; `npm root -g` (para ubicar el paquete instalado); `grep -rl -E "registerGatewayMethod|registerControlUiDescriptor|registerHttpRoute|backupResources|stateDir|dataDir" <raíz-del-paquete-openclaw>/dist`; y `ls` del directorio de estado de summa-gate en ese host. Todos de lectura; cualquier otro comando en ese turno está fuera de la lista.
