...
4. **Muse (carril D):** `cd /Users/dn/dev/wt-f7-D && mkdir -p .saikit/scratch/D && ~/.local/bin/muse-bin-1.3.0-R3057.1 exec --prompt-file BRIEF.md --json --reasoning-effort max --yolo --workspace /Users/dn/dev/wt-f7-D > .saikit/scratch/D/muse.jsonl`. `--yolo` desactiva aprobaciones y sandbox **solo** para ese worktree desechable; el brief le prohíbe salir del worktree. El JSONL es la evidencia de la corrida y vive dentro de la zona del carril (`.saikit/scratch/D/`).
