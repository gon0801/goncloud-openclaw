...
- [ ] **Q0 El PR #46, solo si el paso 0.0 lo encontró abierto.** Es docs ya redactado y revisado: loop reducido (CI verde + CodeRabbit con la regla de 20 min + `APPROVE lead <sha>`, sin revisor cruzado), ventana segura, ruta del kit. Hasta que esté en `main`, los carriles pueden trabajar (leen el runbook desde `origin/docs/fase7-tablero-runbook`), pero nada de ellos se mergea antes que Q0.
...
| "Reload" después de un `config patch` | No hay comando aparte: el `config patch` recarga la config del gateway por sí mismo (por eso mata runs en vuelo). Si 2 min después `openclaw plugins list` no refleja el cambio, se reinicia el gateway con `scripts/restart-openclaw-gateway.ps1` vía exec del gateway, lanzado desacoplado (`Start-Process ... -WindowStyle Hidden`, si no muere con el nodo que lo disparó), con las mismas condiciones de cero runs en vuelo y ningún cron con `Next` en 15 min. |
