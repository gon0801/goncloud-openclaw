...
El gate exige un veredicto sellado por el harness saikit de la sesión: por eso la instrucción de arranque del lead lleva el sentinel `-saikit:autopilot` (ver pie de página). Si el gate responde "sin estado del hook" o "sin veredicto sellado", no hay otra ruta de merge: fila correspondiente de la tabla de atores. El sellado del veredicto y la corrida de `saikit-merge.sh` ocurren en la misma sesión viva, porque un Stop limpio del turno borra el estado del hook que el gate cruza.
...
| La ruta de merge del kit rechaza (hash del manifiesto, sin CI reconocido, lock ajeno con exit 3) | El binario `summonaikit` no existe en esta Mac (command not found, medido 2026-09-15); el slash command `/saikit-update` existe pero está roto por eso mismo y no recupera el merge. Las herramientas reales son `~/.claude/saikit-tools/` (`saikit-blast.sh`, `saikit-decision.sh`) y `/Users/dn/dev/summonaikit-claude/tools/`. Una vez: reintento del gate. Si sigue: ese PR queda abierto con su `APPROVE lead <sha>` y la razón textual del gate, y se lista en el Telegram. No se usa ninguna otra ruta de merge (ni `gh pr merge`, ni API, ni `--liberar-lock` de un lock que no es tuyo). |
| La ruta de merge del kit rechaza con "sin veredicto sellado" o hay commits después del veredicto | El veredicto `.saikit/veredictos/<sha>.json` lo escribe el subagente reviewer con la tool Write (el hook solo sella el sha256 de ESE Write; un archivo escrito a mano, con Bash o con Edit queda en disco pero no sella). Recuperación: despachar UN reviewer con pedido de veredicto sellado contra el head actual, re-revisando solo el delta desde la última revisión (la batería y el blast ya corridos se citan en el despacho; NO se repite la ceremonia entera). El closer de sellado NO se arma con `-saikit:fast`: ese carril exime la ceremonia de subagentes y el turno puede cerrar declarando un sello que nunca ocurrió (medido 2026-09-15 en wt-G: el tsv declaraba "sellado ok" con agents_seen vacío y sin veredicto en disco). Si sigue: ese PR queda abierto con su `APPROVE lead <sha>` y la razón textual del gate, y se lista en el Telegram. No se usa ninguna otra ruta de merge (ni `gh pr merge`, ni API, ni `--liberar-lock` de un lock que no es tuyo). |
