...
| Rol | Quién | Qué hace |
|---|---|---|
| **claw** | el agente `main` del gateway | Crea el worktree del lead, lo lanza, lo vigila por tmux (la sesión va marcada con `OPENCLAW_WATCH`) y lo relanza si se cae. Hace la limpieza del cierre que el lead no puede hacerse a sí mismo. No mergea ni despliega. |
...
| **David** | el dueño | **Mergea cada PR** con el comando que el lead le deja en el PR: summonaikit-claude no tiene `.saikit/autopilot.json`, así que la ruta del kit da NO-MERGE y el lead nunca mergea. Ninguna otra acción durante la corrida. |
...
5. **Commits**: pre-commit siempre, jamás `--no-verify`; el scope es un área (`fix(hook): …`), nunca un número de fila. El literal `gh pr merge` no va en ningún comando (el hook lo niega), y `git push` va en un comando aparte de `gh pr create --base master`. Los carriles del lead commitean su rastro (`.saikit/decisiones/fase<N>-<carril>.tsv` y `.saikit/findings/blast-fase<N>-<carril>.json`): el repo lo versiona.
...
7. **El merge es de David.** Con `gate` en `pass` y el `APPROVE lead <sha>` puesto, el lead deja el comando sin escribir el literal y marca `atencion_requerida` en el progreso:

   ```
   printf '! gh pr %s %s --repo gon0801/summonaikit-claude --merge --delete-branch --match-head-commit %s\n' merge <pr> <sha> > /tmp/f<N>-saikit/merge-<pr>.txt
   /opt/homebrew/bin/gh pr comment <pr> --repo gon0801/summonaikit-claude --body-file /tmp/f<N>-saikit/merge-<pr>.txt
   ```

   La espera no tiene tope: `until [ "$(/opt/homebrew/bin/gh pr view <pr> --repo gon0801/summonaikit-claude --json state --jq .state)" = MERGED ]; do sleep 300; done` (en segundo plano, o en tramos de 540 s). Mientras espera, sigue con los carriles que no dependen de ese merge. Tras el merge: el deploy (regla 6) si toca, y la limpieza del carril en este orden: `$T kill-session -t <token>-wt-f<N>-saikit-<carril>` (lanzar la marcó, y `cierre-de-fase.sh` cuenta toda sesión marcada), `git -C /Users/dn/dev/summonaikit-claude worktree remove <worktree>` (si se niega por archivos sin versionar: `git -C <worktree> status --porcelain > /tmp/f<N>-saikit/<carril>/restos.txt` y se repite con `--force`), y `branch -D fase<N>/<carril>` en el checkout principal (el `--delete-branch` borra la remota). En un carril del lead, salvo el de cierre (su rama la quita claw, «Cierre»), no hay sesión ni worktree que quitar: solo `checkout --detach origin/master` en su worktree y después `branch -D`; el `--force` nunca va sobre `/Users/dn/dev/wt-f<N>-saikit-lead` ni sobre `/Users/dn/dev/wt-f<N>-saikit-deploy`.
...
- **§6**: no hay ruta de merge del kit; mergea David (regla 7).
...
| David no mergea | El carril espera sin tope; los demás siguen; el progreso mantiene `atencion_requerida`. |
