...
**One stopping rule:** deliver after that round, with every remaining one-sentence item written into the runbook or declared as a residual in the PR. Do not deliver on an unverified pass, and do not keep going past the cap hoping for an empty list: every full rewrite manufactures new contradictions for the next reader, which is how the Fase 7 runbook reached thirteen passes and the Fase 10 runbook five versions without ever emptying. If the cap leaves a mechanism unwritten, the document is too long: move the shared part into the base runbook or into a script and verify that once.
