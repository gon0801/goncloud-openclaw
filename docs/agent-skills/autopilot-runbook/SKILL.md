...
16. **Promesa observable.** Every plan row (`Plans.md`) a lane executes carries one literal line inside its Contenido cell, in this exact form: `Promesa: <qué vería un humano si funciona> — ruta: <cómo llegaría ahí>.` A row with nothing a user could see writes `Promesa: sin promesa observable.` instead, and that is accepted — what is not accepted is leaving the question unanswered. This line is the ONLY input the `usuario` agent receives (`agents/usuario/agent/AGENTS.md`, Fase 9.11): it never reads the diff, the tests or the PR, only the promise and the route. `scripts/cierre-de-fase.sh` (Fase 9.12) parses ONLY this literal `Promesa: … — ruta: …` line, never prose elsewhere in the row: for every row that declares one, it requires a matching `## <fase>.<tarea>` block ending in a `FUNCIONA` line under `docs/evidence/usuario-<fase>-*.md`, and names the row when that is missing. Measured 2026-09-17, Fase 7: the owner-facing dashboard shipped, tested green, and nobody ever opened it — this slot is what gives the `usuario` agent something to open, and what keeps the fase from closing on a promise nobody checked.
<!-- candado: test-skill-autopilot-runbook.sh -->
...
| Does the tool the runbook relies on have a precondition, and is it durable — a merged config file, an installed binary, a receipt comment on the PR — rather than session, hook or host state? | Arranque or Cola: name the precondition and the command that checks it against the artifact itself; approval reads the PR receipt (`saikit-entrega.v1`), never a session or sealed state |
...
| For each task, what would a USER see if it worked — and who opens it? | Slot 16, Promesa observable: the exact `Promesa: … — ruta: …` line per row, or `Promesa: sin promesa observable.`. That line is the whole input of the `usuario` agent; it never reads the diff. Measured 2026-09-17: the Fase 7 shipped a dashboard for the owner to watch a phase; it passed its tests, CI was green, three PRs landed — and nobody ever switched it on or opened it |
