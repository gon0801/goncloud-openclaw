...
Dispatch one fresh-context subagent as the executor with this SKILL.md, the base runbook and the phase runbook, and the instruction: "run every read-only command in the document, then list every question you would still have to ask a human; if the answer is deducible but not written, list it". Fix the runbook for every item of mechanism; write the one-sentence items in. Then one reviewer of the diff (fresh context, read-only) for contradictions between paragraphs. **That is the whole budget: one reader, one reviewer, one round of fixes.** A third pass is allowed only when the reviewer found a mechanism item, and it is the last one.

**One stopping rule:** deliver after that round, with every remaining one-sentence item written into the runbook; anything left unfixed goes to a row of the plan (Plans.md or the repo tracker) and is named in the PR. Only a blocking item (the executor would fail or do harm, with the command that proves it) earns the third pass. Do not deliver on an unverified pass, and do not keep going past the cap hoping for an empty list: every full rewrite manufactures new contradictions for the next reader, which is how the Fase 7 runbook reached thirteen passes and the Fase 10 runbook five versions without ever emptying. If the cap leaves a mechanism unwritten, the document is too long: move the shared part into the base runbook or into a script and verify that once.
